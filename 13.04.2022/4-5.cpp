...
public:
	...
	list<T> erase_before(iterator& it)
	{
		list<T> copy = *this;
		this->erase_before_inplace(it);
		std::swap(copy, *this);
		return copy;
	
	}
	void erase_before_inplace(iterator& it)
	{
		while (this->begin() != it)
		{
			this->pop_front();
...
int main()
{
	...
	list<int> b = a.erase_before(++++a.begin());
	std::cout << "Copy erase: ";
	std::cout << "\na: ";
	for (int& x : a)
		std::cout << x << ' ';
	std::cout << "\nb: ";
		for (int& x : b)
			std::cout << x << ' ';
	std::cout << "\nInplace erase: ";
	a.erase_before_inplace(++++a.begin());
	b = a;
	std::cout << "\na: ";
	for (int& x : a)
		std::cout << x << ' ';
